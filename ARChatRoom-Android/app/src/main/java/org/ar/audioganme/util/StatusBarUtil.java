...
public class StatusBarUtil {
     ...
     * 沉浸状态栏
     */
    public static void setStatusBar(boolean isChange, Activity mActivity, int colorId) {
        if (!isChange){
            return;
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
...
//            mActivity.getWindow().setStatusBarColor(mActivity.getResources().getColor(R.color.window_status_bar));
    ...
    /**
     *将状态栏元素设置为深色（为了解决背景色是白色状态栏元素看不见问题）
     *
     */
    public static boolean setDeepStatusBar(boolean isChange, Activity mActivity, int colorId) {
        if (!isChange){
            return false;
        }
